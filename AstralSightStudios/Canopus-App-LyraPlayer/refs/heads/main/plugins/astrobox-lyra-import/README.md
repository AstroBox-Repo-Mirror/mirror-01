...
- 本地文件按内容识别类型，不依赖文件名后缀（安卓选择器只提供无后缀的 content URI 名称）：MP3 以 MPEG Layer III 帧头判断（跳过 ID3v2 标签），封面以 JPEG/PNG 文件头判断，歌词按内容区分 JSON 与 LRC；选择 MP3 后从 ID3v2/ID3v1 标签预填曲名、歌手和专辑；
